...
def extract_device_name(point_name):
    ...
    for suffix in all_suffixes:
        if point_name.endswith(suffix):
            base = point_name[:-len(suffix)]
            # 如果base非空且有意义，返回设备名
...
def q_addr_to_ka_num(q_addr, pin):
    """将Q地址转换为KA继电器编号
    Q2.0 -> KA1
    Q2.1 -> KA2
    Q2.2 -> KA3
    ...
    pin: 5 或 9
    """
    if not q_addr or not q_addr.startswith('Q'):
        return None
    
    try:
        parts = q_addr[1:].split('.')
        byte = int(parts[0])
        bit = int(parts[1])
        
        # 计算是第几个Q点 (假设从Q2.0开始)
        q_index = (byte - 2) * 16 + bit
        
        # 每个Q点对应一个KA
        ka_num = q_index + 1
        
        return f"KA{ka_num}_{pin}"
    except:
        return None


def qw_addr_to_polarity(qw_addr, polarity):
    """将QW地址转换为带极性的地址"""
    if not qw_addr:
        return None
    return f"{qw_addr}{polarity}"


def parse_i_addr(addr):
    """解析I地址为数值用于排序"""
    if not addr or not addr.startswith('I'):
        return None
    try:
        parts = addr[1:].split('.')
...
def generate_terminal_rows_with_spacing(devices, add_spacing=True, priority_dict=None,
    ...
    # 按设备类型分组排序
    has_i_devices = []      # 有I的设备
    iw_only_devices = []    # 只有IW的设备（无I）
    qw_only_devices = []   # 只有QW的设备（无I且无IW）
    ...
    for device, points in devices.items():
        ...
        if has_i:
            ...
            has_i_devices.append((min_addr if min_addr else 999999, device, points))
        elif has_iw:
            # 只有IW的设备
    ...
    # 按顺序处理各组设备，KA编号全局连续
    ka_counter = [1]  # 使用列表实现可变引用
    
    for item in has_i_devices:
        device = item[1]
        points = item[2]
        _process_device(rows, device, points, ka_counter, add_spacing, plc_type, power_pos, power_neg, priority_dict)
...
def _process_device(rows, device, points, ka_counter, add_spacing=True,
    ...
    for key in _sorted_keys(q_pts, priority_dict):
        rows.append((seq, f"{key}+", f"KA{ka_counter[0]}_5", None))
        seq += 1
        rows.append((seq, f"{key}-", f"KA{ka_counter[0]}_9", None))
        seq += 1
        ka_counter[0] += 1
    ...
    for key in _sorted_keys_with_polarity(iw_pts, priority_dict, plc_type):
        if plc_type == "1500":
            ...
            rows.append((seq, key, iw_pts[key], None))
            seq += 1
            ...
            # 200SMART模式：分+-和24V
            rows.append((seq, f"{key}", f"{iw_pts[key]}", ""))
            seq += 1
            rows.append((seq, power_pos, power_pos, ""))
            seq += 1
            rows.append((seq, power_neg, power_neg, ""))
    ...
    for key in _sorted_keys_with_polarity(qw_pts, priority_dict, plc_type):
        qw_addr = qw_pts[key]
        rows.append((seq, f"{key}", f"{qw_addr}", None))
        seq += 1
...
if __name__ == '__main__':
    generate_terminal_excel('PLC点位表.xlsx', '端子表_生成.xlsx')
